...
public class ClusterMissileDeathComponent extends DeathComponent {
    ...
    public void die(NEntity entity){
        ...
        ProjectileIndustry pri = (ProjectileIndustry) Engine.getEngine().getIndustryMap().get("cluster_shrapnel_projectile_industry");
        float rot = 0;
        for (int i = 0; i < 6; i++){
            NEntity projectile = pri.produce(new PositionComponent(pc.getCoord(), rot), (LinkComponent)entity.getComponent(ComponentType.LINK));
